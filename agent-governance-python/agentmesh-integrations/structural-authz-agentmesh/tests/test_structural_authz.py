...
class TestAuthzGate:
    ...
    def test_denied_missing_scope(self):
        ...
        decision = gate.evaluate(agent, "admin task", artifact, required_scopes=["admin"])
        assert not decision.allowed
...
class TestDelegationChain:
    ...
    def test_expired_link_error(self):
        chain = DelegationChain("did:authz:root", ["read"])
        link = self._unsigned_link("did:authz:root", "did:authz:agent", ["read"], expires_at=_past())
        chain.add_link(link)
        ok, reason = chain.validate()
        assert not ok
    ...
    def test_scope_widening_error(self):
        chain = DelegationChain("did:authz:root", ["read"])
        link = self._unsigned_link("did:authz:root", "did:authz:agent", ["read", "admin"])
        chain.add_link(link)
    ...
    def test_signed_chain_validates(self):
        import base64
        from cryptography.hazmat.primitives.asymmetric import ed25519 as _ed_module

        root_priv, root_pub = generate_keypair()
        root_priv_obj = _ed_module.Ed25519PrivateKey.from_private_bytes(base64.b64decode(root_priv))
    ...
    def test_signed_chain_with_gate(self):
        ...
        decision = gate.evaluate(root_agent, "read task", artifact, required_scopes=["read"], delegation_chain=chain)
        assert decision.allowed
...
class TestIntegration:
    def test_full_authz_lifecycle_no_crypto(self):
        ...
        decision = gate.evaluate(agent, "process data", artifact, required_scopes=["read"])
        assert decision.allowed
        ...
        artifact_revoked = _make_artifact(did="did:authz:worker", grade=TrustGrade.REVOKED)
    ...
    def test_scope_chain_propagation(self):
        ...
        assert chain.effective_scopes_for("did:authz:root") == {"read", "write", "admin"}
